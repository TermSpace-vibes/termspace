      ...
      <div style={{ padding: '4px 0', flexShrink: 0, display: 'flex', justifyContent: 'center' }}>
        <button
          ...
          style={{
            ...
            cursor: 'pointer', color: isPlaying ? 'var(--accent)' : 'var(--text-dim)',
  ...
  return (
    <div style={{ padding: '10px 10px', flexShrink: 0 }}>
      <div style={{
        ...
        border: '1px solid var(--border-inactive)', borderRadius: 14, padding: 12,
      }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, minWidth: 0, overflow: 'hidden' }}>
            {current.isPlaying && !reducedMotion ? (
              ...
              <span style={{ width: 5, height: 5, borderRadius: '50%', background: current.isPlaying ? 'var(--accent)' : 'var(--text-dim)', flexShrink: 0 }} />
            )}
            <span style={{
              fontSize: 10, letterSpacing: 1.2, color: 'var(--text-dim)', fontWeight: 600, textTransform: 'uppercase',
          ...
          <span style={{
            fontSize: 10, color: 'var(--text-dim)', whiteSpace: 'nowrap', textOverflow: 'ellipsis',
            overflow: 'hidden', maxWidth: 100, flexShrink: 0, marginLeft: 6,
          }}>
            ...
            >
              {current.thumbnailUrl ? (
                <img
                  ...
                  alt=""
                  style={{ width: 48, height: 48, borderRadius: 10, objectFit: 'cover', flexShrink: 0, border: '1px solid var(--border-inactive)' }}
                  onError={(e) => { e.currentTarget.style.display = 'none' }}
                />
              ) : (
                <div style={{
                  width: 48, height: 48, borderRadius: 10, background: 'var(--bg-main)', flexShrink: 0,
                  display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid var(--border-inactive)',
                }}>
              ...
              <div style={{ minWidth: 0, flex: 1, overflow: 'hidden' }}>
                <div style={{
                  fontSize: 13, fontWeight: 600, color: 'var(--text-active)', lineHeight: '15px',
                  display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden',
                }}>
                  {current.mediaTitle || current.pageTitle || current.pageUrl}
                </div>
                <div style={{ fontSize: 11, color: 'var(--text-dim)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginTop: 3 }}>
                  {subtitle}
                </div>
              </div>
            </motion.div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
          ...
          >
            {current.isPlaying ? <Pause size={16} fill="var(--bg-sidebar)" /> : <Play size={16} fill="var(--bg-sidebar)" style={{ marginLeft: 1 }} />}
          </button>
...
const ghostBtnStyle: CSSProperties = {
  width: 26, height: 26, background: 'transparent', border: 'none', borderRadius: 7,
  color: 'var(--text-dim)', cursor: 'pointer', display: 'flex', alignItems: 'center',
  justifyContent: 'center', flexShrink: 0, transition: 'background 0.15s ease, color 0.15s ease',
  overflow: 'hidden',
}

const switcherBtnStyle: CSSProperties = {
  width: 22, height: 22, background: 'transparent', border: 'none', borderRadius: 6,
  color: 'var(--text-dim)', cursor: 'pointer', display: 'flex', alignItems: 'center',
  justifyContent: 'center', flexShrink: 0, transition: 'background 0.15s ease, color 0.15s ease',
}
...
const playBtnStyle: CSSProperties = {
  width: 34, height: 34, background: 'var(--accent)', border: 'none', borderRadius: '50%',
  color: 'var(--bg-sidebar)', cursor: 'pointer', display: 'flex', alignItems: 'center',
  justifyContent: 'center', flexShrink: 0, boxShadow: '0 2px 10px -2px color-mix(in srgb, var(--accent) 60%, transparent)',
